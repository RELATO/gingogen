...
var rootCmd = &cobra.Command{
	Use:   "gingogen",
	Short: "gingogen is a scaffold tool for Gin and GORM, one command to generate a mighty RESTful APIs App",
	Long: `
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/gingogen.yaml)")
...
func initConfig() {
	if cfgFile != "" {
		...
		// Search config in home directory with name "gingogen" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName("gingogen")
	}
