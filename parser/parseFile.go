...
func (p *ParseEngine) SaveFlagsToUserConfigFile() error {
	...
	tplPath := path.Join(goPath, "src", "github.com/relato/gingogen", "tpl/cobra.gingogen.yaml")
	tmpl, err := template.ParseFiles(tplPath)
	if err != nil {
		return err
	}
	outPath := path.Join(home, "gingogen.yaml")
...
func (p *ParseEngine) copyStaticSwaggerFolder() {
	...
	for _, vv := range tasks {
		src := path.Join(goPath, "src", "github.com/relato/gingogen/boilerplate", vv)
